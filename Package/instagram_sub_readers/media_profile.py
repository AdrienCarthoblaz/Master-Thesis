...
class InstagramProfileReader(Reader):
    ...
    def read(self):
        '''
        Same as above but for profile picture, changes :
            --> the dictionary cannot be "found" as there are 'true' in the json file. This function found their
                locations and transform them in a string 
        '''
        ...
        list_profile = []
        for i in data[index[1]:index[2]]:
            list_profile.append(i)
        
        occurences = lambda s, lst: (i for i,e in enumerate(lst) if e == s)
        index_true = list(occurences('true,', list_profile))
        ...
        main_df['label'] = main_df.date.apply(lambda x: 'profile')
