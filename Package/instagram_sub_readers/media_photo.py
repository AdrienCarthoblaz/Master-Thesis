...
class InstagramPhotoReader(Reader):
    ...
    def read(self):
        '''
        Same as above but for photos changes as follow: 
            --> index is now between 'photos' and 'profile'
            --> in the string transformation : 1) a '{' is added at the beginning 2) drop the coma 3) add '}' at the end 
        '''
        ...
        list_photos = []
        for i in data[index[0]:index[1]]:
            list_photos.append(i)
        ...
        main_df['label'] = main_df.date.apply(lambda x: 'photos')
